...
import com.sshp.plugins.hibernate.select.ResolveFilter;
...
public class SQLProjection implements Projection {
  ...
  private String replace(String sql, Criteria criteria, CriteriaQuery criteriaQuery) {
    ...
    this.alias.keySet().iterator().forEachRemaining(item -> alias.put(resolveFilter.generateAlias(this.alias.get(item)), item));
  ...
  public void setResolveFilter(ResolveFilter resolveFilter) {
    setAlias(resolveFilter.aliasMap);
